...
import { MAILBOX_DRAFT_KEY } from "@/utils/supabase";
...
export default function LetterComposer({replyTo,onSent,onCancel}:{replyTo?:Letter|null;onSent:()=>void;onCancel:()=>void}){
  const {books,films,cps}=useArchiveData(); const {members,member,sendLetter,setTyping}=useMailbox();
  ...
  useEffect(()=>{setTyping(Boolean(content.trim()));const timer=window.setTimeout(()=>setTyping(false),4500);return()=>{window.clearTimeout(timer);setTyping(false);};},[content,setTyping]);
  ...
  return <div className="letter-composer-backdrop" role="presentation" onMouseDown={(event)=>event.target===event.currentTarget&&onCancel()}><section className="letter-composer" role="dialog" aria-modal="true" aria-label={replyTo?"写一封回信":"写一封信"}><button className="letter-composer__close" type="button" onClick={onCancel} aria-label="关闭写信面板">×</button><header><small>OWL POST · PRIVATE DRAFT</small><h2>{replyTo?"写一封回信":"写一封信"}</h2><p>收信人 · {recipient?.display_name??"远方的另一位收信人"}</p></header>{replyTo?<div className="reply-reference">回应 {replyTo.author?.display_name} 在 {new Date(replyTo.created_at).toLocaleDateString("zh-CN")} 写下的信</div>:null}<form onSubmit={submit}><div className="composer-options"><label>信件类型<select value={letterType} onChange={(event)=>setLetterType(event.target.value as LetterType)}>{types.map((item)=><option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label>关联本地档案<select value={attachmentKey} onChange={(event)=>setAttachmentKey(event.target.value)}><option value="">不关联</option>{attachmentOptions.map((item)=><option key={item.key} value={item.key}>{item.attachment.title}</option>)}</select></label></div><label className="composer-paper"><span>亲爱的 {recipient?.display_name??"收信人"}：</span><textarea autoFocus required maxLength={2000} value={content} onChange={(event)=>setContent(event.target.value)} placeholder="写下只属于这座邮局的话……"/><small>{content.length} / 2000</small></label><fieldset className="mood-stamps"><legend>选择心情印章</legend>{stamps.map((item)=><button className={stamp===item.value?"is-active":""} type="button" key={item.value} onClick={()=>setStamp(stamp===item.value?null:item.value)}>{item.symbol}</button>)}</fieldset><footer><span>草稿仅保存在当前浏览器，投递后才会离开这里。</span><button className="owl-primary" disabled={busy||!content.trim()} type="submit">{busy?"猫头鹰正在起飞……":"投递给猫头鹰"}</button></footer></form></section></div>;
}

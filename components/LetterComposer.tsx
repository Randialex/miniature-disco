...
export default function LetterComposer({ replyTo, onSent, onCancel }: { replyTo?: Letter | null; onSent: () => void; onCancel: () => void }) {
  ...
  const { members, member, sendLetter } = useMailbox();
  ...
  return <div className="letter-composer-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onCancel()}>
    <section className="letter-composer" role="dialog" aria-modal="true" aria-label={replyTo ? "写一封回信" : "写一封信"}>
      ...
      <header><small>NOCTUA POST · LETTERS AFTER MIDNIGHT</small><h2>{replyTo ? "写一封回信" : "投一封夜枭来信"}</h2><p>魔法署名 · {member?.display_name}　收信人 · {recipient?.display_name ?? "档案馆主"}</p></header>
      ...
      <form onSubmit={submit}>
        ...
        <label className="composer-paper"><span>亲爱的 {recipient?.display_name ?? "馆主"}：</span><textarea autoFocus required maxLength={4000} value={content} onChange={(event) => setContent(event.target.value)} placeholder="把未说完的话，投进会记得你的夜色里。" /><small>{content.length} / 4000</small></label>
        ...
        <footer><span>{replyTo ? "回信会叠放在原信下方。" : draftSavedAt ? `草稿已于 ${new Date(draftSavedAt).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })} 留在本机` : "停止书写 650ms 后自动保存本机草稿。"}</span><button className="owl-primary" disabled={busy || !content.trim()} type="submit">{busy ? "已进入夜枭投递队列……" : replyTo ? "寄出回信" : "投递这封信"}</button></footer>
      </form>
    </section>
  </div>;
}

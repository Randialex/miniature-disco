...
export function EditorSection({ title, description, children, tone = "default" }: {
  title: string;
  description?: string;
  children: ReactNode;
  tone?: "default" | "danger";
}) {
  return <section className={`editor-section${tone === "danger" ? " editor-section--danger" : ""}`}>
    <header className="editor-section__heading">
      <div><h3>{title}</h3>{description ? <p>{description}</p> : null}</div>
    </header>
    <div className="editor-section__grid">{children}</div>
  </section>;
}

export function Field({ label, children, wide = false, span, hint }: {
  label: string;
  children: ReactNode;
  wide?: boolean;
  span?: 4 | 6 | 12;
  ...
  const fieldSpan = wide ? 12 : span ?? 6;
  return <label className={`editor-field editor-field--span-${fieldSpan}`}>
    <span>{label}</span>
    {children}
    {hint ? <small className="editor-field__hint">{hint}</small> : null}
  </label>;
}
...
export function RelationPicker({ label, items, value = [], onChange }: {
  ...
  return <fieldset className="relation-picker"><legend>{label}</legend>{items.length ? <><input className="relation-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索标题、作者或人物" /><div>{visible.map((item) => <label key={item.id} title={item.label}><input type="checkbox" checked={value.includes(item.id)} onChange={() => toggle(item.id)} /><span>{item.label}</span>{value.includes(item.id) ? <small>已关联</small> : null}</label>)}</div>{!visible.length ? <p>没有找到匹配档案</p> : null}</> : <p>暂无可关联档案</p>}</fieldset>;
}

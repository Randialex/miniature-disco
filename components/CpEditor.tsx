...
import EditorFrame, { Field, makeId, RelationPicker, listToText, textToList } from "./EditorFrame";
...
export default function CpEditor({ cps, onClose, onCommit }: { cps: Cp[]; onClose: () => void; onCommit: (items: Cp[]) => boolean }) {
  const { books, films } = useArchiveData();
  ...
  const save = () => { if (onCommit(draft)) { setStatus("已进入安全同步队列"); window.setTimeout(onClose, 450); } else setStatus("保存失败：当前身份只读或本机存储不可用"); };

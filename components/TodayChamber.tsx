...
export default function TodayChamber() {
  ...
  const firstOpen = typeof window !== "undefined" && !window.sessionStorage.getItem(`randi-chamber-open:${activeArchive.id}:${dayKey()}`);
  if (firstOpen && typeof window !== "undefined") window.sessionStorage.setItem(`randi-chamber-open:${activeArchive.id}:${dayKey()}`, "1");
  ...
  return <section className={`today-chamber${firstOpen ? " today-chamber--unsealing" : ""}`} aria-labelledby="chamber-title">
    ...
    <div className="chamber-grid">
      {sections.map((section) => <article className="chamber-column" key={section.id}><header><span>{section.icon}</span><div><small>{section.latin}</small><h3>{section.title}</h3><p>{section.copy}</p></div></header><div>{section.items.map((item) => <ChamberCard key={item.id} item={item} onDismiss={() => dismiss(item.id, "later")} />)}</div></article>)}

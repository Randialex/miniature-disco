...
export default function ArchiveComments({ kind, legacyId, title, anchors = [], onPromote }: ArchiveCommentsProps) {
  ...
  useEffect(() => {
    ...
    const recent = window.localStorage.getItem("randi-visitor-nickname-v1") ?? "";
    setNickname((current) => current || recent);
  }, [user.is_anonymous]);
  ...
  async function submit(event: FormEvent) {
    ...
    if (saved) {
      ...
      setReplyTo(null);
      setContent("");
      if (user.is_anonymous) window.localStorage.setItem("randi-visitor-nickname-v1", nickname.trim());
      setNotice("旁注已夹进这份档案");
    }
  }
  ...
  return <section className="archive-comments" aria-labelledby={`comments-${kind}-${legacyId}`}>
    ...
    {composerOpen ? <div className="comment-composer-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setComposerOpen(false)}><form className="comment-composer" onSubmit={submit} role="dialog" aria-modal="true" aria-label={replyTo ? "回复旁注" : "写访客旁注"}>
      ...
      <header><small>MARGINALIA · ANCHORED RESPONSE</small><h3>{replyTo ? `回信给 ${replyTo.author?.displayName ?? "访客"}` : "写一条访客旁注"}</h3></header>
       {user.is_anonymous ? <label className="visitor-signature"><span>魔法署名</span><input autoFocus required maxLength={32} value={nickname} onChange={(event) => setNickname(event.target.value)} placeholder="例如：月见、拾梦人、无名旅客" /><small>不需要邮箱；这个昵称只用于来信与旁注。</small></label> : null}
      ...
      {quotedText ? <blockquote>“{quotedText}”</blockquote> : null}
       <label className="comment-composer__paper"><span>把未说完的话写在档案页边：</span><textarea autoFocus={!user.is_anonymous} required maxLength={4000} value={content} onChange={(event) => setContent(event.target.value)} placeholder="写给这一次阅读、观看，或某个仍未熄灭的场面……" /><small>{content.length} / 4000</small></label>
       <footer><span>旁注会保留锚点与可见范围。</span><button className="owl-primary" type="submit" disabled={busy || !content.trim() || (user.is_anonymous && !nickname.trim())}>{busy ? "正在夹入档案……" : replyTo ? "寄出回信" : "保存旁注"}</button></footer>
    </form></div> : null}
  </section>;
}

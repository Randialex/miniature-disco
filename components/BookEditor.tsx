...
import EditorFrame, { Field, listToText, makeId, textToList } from "./EditorFrame";
...
export default function BookEditor({ books, onClose, onCommit }: { books: Book[]; onClose: () => void; onCommit: (items: Book[]) => boolean }) {
  ...
  return <EditorFrame title="书录编辑术" tone="book" onClose={onClose} onSave={save} status={status}>
    ...
    {book ? <div className="editor-form">
      ...
      <Field label="关联影视 ID（逗号分隔）"><input value={book.filmIds?.join("，") ?? ""} onChange={(e) => patch({ filmIds: textToList(e.target.value) })} /></Field>
      <Field label="关联 CP ID（逗号分隔）"><input value={book.cpIds?.join("，") ?? ""} onChange={(e) => patch({ cpIds: textToList(e.target.value) })} /></Field>

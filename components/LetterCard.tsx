...
export default function LetterCard({ letter, replies, onReply }: { letter: Letter; replies: Letter[]; onReply: (letter: Letter) => void }) {
  const { member, mailbox, editLetter, deleteLetter, togglePin, toggleReaction } = useMailbox();
    ...
    <article className={`owl-letter owl-letter--${letter.author?.role ?? "guest"}${letter.is_pinned ? " is-pinned" : ""}${letter.status === "pending" ? " is-pending" : ""}`}>
      ...
      <footer>
        ...
        <div className="letter-actions">
          ...
          {own ? <button type="button" onClick={() => deleteLetter(letter.id)}>收回</button> : null}
          <button type="button" onClick={() => togglePin(letter)}>{letter.is_pinned ? "取消置顶" : "置顶"}</button>
        </div>
      </footer>
      {replies.length ? (

...
export default function ArchiveNotes({ notes = [], onChange }: { notes?: ArchiveNote[]; onChange: (notes: ArchiveNote[]) => boolean }) {
  ...
  const sorted = editing ? draft : [...notes].sort((a,b) => Number(Boolean(b.pinned))-Number(Boolean(a.pinned)) || (a.order ?? 9999) - (b.order ?? 9999) || b.createdAt.localeCompare(a.createdAt));
  ...
  const move = (id:string,direction:-1|1) => setDraft((items) => { const index=items.findIndex((item)=>item.id===id); const target=index+direction; if(index<0||target<0||target>=items.length)return items; const next=[...items]; [next[index],next[target]]=[next[target],next[index]]; return next.map((item,order)=>({...item,order})); });

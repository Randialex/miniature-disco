...
export function ArchiveDataProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getUser().then(({ data: authData }) => {
      ...
      setUser(nextUser);
      setAuthChecked(true);
      if (nextUser) void bootstrap(nextUser).catch((cause: unknown) => {
        setReady(true);
        setSyncState("error");
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      setUser(nextUser);
      setAuthChecked(true);
      if (nextUser && event === "SIGNED_IN") void bootstrap(nextUser);
      if (!nextUser) {
  ...
  const value = useMemo<ArchiveContextValue | null>(() => user && activeArchive ? ({
    ...
    updateMember,
    restoreBackup,
  }) : null, [activeArchive, archives, backups, conflicts, data, enqueueDataChange, entries, getEntryHistory, invitations, lastSyncedAt, members, pendingCount, ready, refreshCloud, resolveConflict, restoreBackup, restoreTrashEntry, restoreVersion, revokeInvitation, switchArchive, syncMessage, syncState, supabase, trash, updateMember, user, createInvitation]);

...
import { useArchiveSocial } from "@/components/ArchiveSocialProvider";
...
const navigation = [
  ...
  { href: "/owl-post", label: "夜枭来信", tone: "mailbox" },
...
export default function Navbar() {
  ...
  const { unreadCount } = useArchiveSocial();
  ...
  return (
    <header className={`navbar${scrolled ? " navbar--scrolled" : ""}`}>
      ...
      <button className="notification-trigger" type="button" onClick={() => window.dispatchEvent(new Event("open-randi-notifications"))} aria-label={`打开通知信匣${unreadCount ? `，${unreadCount} 封未读` : ""}`} title="通知信匣"><span aria-hidden="true">✉</span>{unreadCount ? <b>{unreadCount > 9 ? "9+" : unreadCount}</b> : null}</button>
      <button className="search-trigger" type="button" onClick={() => window.dispatchEvent(new Event("open-randi-search"))} aria-label="打开全局搜索" title="全局搜索 Ctrl+K">⌕</button>
      <ThemeToggle />
    </header>
  );
}
